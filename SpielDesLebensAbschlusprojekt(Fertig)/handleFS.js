...
function commitData(user) {
  ...
  data.rasen_fresser.dead =
    data.rasen_fresser.created - data.rasen_fresser.living;
  data.city.dead = data.city.created - data.city.living;
  data.city_destroyer.dead =
    data.city_destroyer.created - data.city_destroyer.living;
...
function deleteUserData(user, waitFor = 0) {
  setTimeout(() => {
    fs.readFile("./data.json", "utf8", function (err, d) {
      if (err) {
        return console.error(err);
...
module.exports = { commitData, countLivings, deleteUserData };
